-- =====================================================
-- CUSTOM FIELD DEFINITIONS MIGRATION (SAFE VERSION)
-- =====================================================
-- This version updates existing functions safely without dropping
-- =====================================================
...
-- =====================================================
-- 2. UPDATE EXISTING HELPER FUNCTION (DON'T DROP IT)
-- =====================================================

-- Update the existing user_belongs_to_company function to handle service role
-- Keep the same parameter name to avoid conflicts
...
-- Create user_has_role helper function for role-based checks
CREATE OR REPLACE FUNCTION user_has_role(target_company_id UUID, allowed_roles TEXT[])
RETURNS BOOLEAN AS $$
BEGIN
  -- If auth.uid() is null, we're using service role - allow access
  IF auth.uid() IS NULL THEN
...
-- =====================================================
-- 3. CREATE CUSTOM FIELD DEFINITIONS TABLE
-- =====================================================
...
-- =====================================================
-- 4. CREATE CUSTOM FIELD AUDIT TABLE
-- =====================================================
