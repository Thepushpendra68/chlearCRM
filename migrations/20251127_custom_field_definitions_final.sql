-- =====================================================
-- CUSTOM FIELD DEFINITIONS MIGRATION (FINAL)
-- =====================================================
...
-- Uses CREATE OR REPLACE for existing functions
-- =====================================================
...
-- =====================================================
-- 5. CREATE HELPER FUNCTIONS FOR RLS (CREATE OR REPLACE)
-- =====================================================

-- Helper function to check if user belongs to company (works with auth.uid() or direct checks)
-- Using CREATE OR REPLACE to avoid dropping existing function
...
-- Using CREATE OR REPLACE to avoid dropping existing function
CREATE OR REPLACE FUNCTION user_has_role(check_company_id UUID, allowed_roles TEXT[])
RETURNS BOOLEAN AS $$
BEGIN
  -- If auth.uid() is null, we're using service role - allow access
  IF auth.uid() IS NULL THEN
