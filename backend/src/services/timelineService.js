...
class TimelineService {
  ...
  async getLeadTimelineSummary(leadId) {
    try {
      ...
      const lastActivityDate = timestamps.length > 0 ? new Date(Math.max(...timestamps)) : null;
      const firstActivityDate = timestamps.length > 0 ? new Date(Math.min(...timestamps)) : null;
