...
/**
 * Import Controller
 * Handles lead import/export operations
...
class ImportController extends BaseController {
  ...
  importLeads = asyncHandler(async (req, res) => {
    ...
    this.success(res, result, 200, `Import completed. ${result.successful_imports} leads imported successfully, ${result.failed_imports} failed.`);
  });

  /**
   * Perform a dry-run validation of the import file and return detailed feedback.
   */
  dryRunLeads = asyncHandler(async (req, res) => {
    if (!req.file) {
      throw new ApiError('No file uploaded', 400);
    }
    ...
    this.success(res, result, 200, `Validation completed. ${result.stats?.valid ?? 0} rows ready to import, ${result.stats?.invalid ?? 0} rows need attention.`);
  });
  ...
  exportLeads = asyncHandler(async (req, res) => {
    ...
    if (format === 'csv') {
      ...
      this.success(res, exportData, 200, 'Export completed successfully');
    }
  });
  ...
  getImportHistory = asyncHandler(async (req, res) => {
    ...
    this.success(res, history, 200, 'Import history retrieved successfully');
  });

  /**
   * Validate import file
   */
  validateImportFile = asyncHandler(async (req, res) => {
    if (!req.file) {
      throw new ApiError('No file uploaded', 400);
    }
    ...
    this.success(res, validation, 200, 'File validated successfully');
   ...
   */
  getFileHeaders = asyncHandler(async (req, res) => {
    if (!req.file) {
      throw new ApiError('No file uploaded', 400);
    }
    ...
    }, 200, 'File headers retrieved successfully');
